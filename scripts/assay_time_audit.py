...
def audit_assay_type() -> None:
    ...
    print("=" * 78)
    for label, column in (("all types (IC50+Ki+Kd+EC50, as deployed)", "pchembl"),
                          ("Ki/Kd only (ATP-independent)", "pchembl_kikd")):
        res = _scaffold_seeds(_cross(column))
        if res["too_small"]:
            print(f"\n{label}\n  n={res['n']} cross-measured — too few to split; "
                  f"no comparable number can be reported.")
            continue
        print(f"\n{label}")
...
def audit_time_split() -> None:
    ...
    print(f"\n  {'cutoff':>8} {'train':>7} {'test':>7} "
          f"{'Spearman':>10} {'direct':>8} {'enrich':>8} {'base':>7}")
    for cutoff in (2015, 2016, 2017, 2018, 2019, 2020):
        ...
        print(f"  {cutoff:>8} {len(tr):>7} {len(te):>7} "
              f"{diff:>10.3f} {direct:>8.3f} {enrich:>8.2f} {base:>7.1%}")
    ...
    print("\n  A time split is strictly harder than a scaffold split: the test "
          "molecules\n  are not just new scaffolds, they are chemistry absent from "
          "the literature\n  at training time. Read the drop, not the absolute value.")
