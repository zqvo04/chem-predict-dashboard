...
> **M1 측정 방식 개정 (2026-08-10, 첫 실행 중).** 원안은 캐스케이드를 "잘린 분자에 `-inf`"로
> 점수화해 95 % 재현율에서 비교하려 했다. 두 번 틀렸다. `roc_auc_score`가 `-inf`를 거부했고
> — 그건 사소하다 — 더 중요하게는 **캐스케이드의 활성 재현율에 천장이 있다.** 게이트가
> 컷 이후 실측 활성 1,928개 중 184개를 자르므로 캐스케이드는 90.5 %를 넘길 수 없고,
> 95 %에서의 "matched recall"은 캐스케이드가 **할 수 없는 일**과 EV를 비교하는 것이다.
>
> 개정: 캐스케이드는 곡선이 아니라 **배포 그대로의 한 점**으로 잰다 —
> `게이트 P >= 임계` **그리고** `pred >= POTENCY_FLOOR`. 그 점의 활성 재현율에 EV를 맞춘다.
>
> **조건 자체(EV의 통과율이 더 낮아야 한다)는 안 바꿨고**, 개정 시점에 알고 있던 것은
> 캐스케이드의 재현율 천장뿐이다 — 어떤 재현율에서도 EV 쪽 숫자는 아직 본 적이 없다.
