...
**2부 모델이 potency 축을 고친다** — `EV = P(binder) · pred + (1−P) · floor`로 채점하면
같은 재적합 위에서 봉인 414 대 실측 활성의 ROC-AUC가 0.890 → 0.956이 된다
(`two_part_audit.py`). **아직 배포에 배선되지 않았다** — 배선은 shortlist 순서를 바꾸므로
gap 축 측정이 먼저다.
